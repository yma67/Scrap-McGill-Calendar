...
with open('username_pswd.txt') as f:
    lines = f.readlines()
...
Select(driver.find_element_by_id("term_input_id")).select_by_visible_text("Winter 2018")
driver.find_element_by_xpath("//option[@value='201801']").click()
driver.find_element_by_xpath("//input[@value='Submit']").click()

"""
# Select All department
for subj in range(1, 242):
    driver.find_element_by_xpath("//*[@id='subj_id']/option["+str(subj)+"]").click()
driver.find_element_by_xpath("/html/body/div[3]/form/input[17]").click()
"""
driver.find_element_by_xpath("//*[@id='subj_id']/option[83]").click()
driver.find_element_by_xpath("/html/body/div[3]/form/input[17]").click()
...
for course in range(1, len(res)+1):
    ...
    try:
        sections = bsobj.find_all("td", text="Lecture")
        if len(sections)==0:
    ...
    for section in sections:
        ...
        try:
            ...
            full_rat = 0
        lecs[tabl[4]]={'title': tabl[7], 'type': tabl[5], 'credit': tabl[6], 'days': tabl[8], 'time': tabl[9], 'capital': tabl[10], 'actual': tabl[11], 'full reg rate': str(full_rat), 'instructor': tabl[16], 'location': tabl[18]}

    course_db[tabl[2] + " " + tabl[3]] = lecs
    driver.execute_script("window.history.go(-1)")
    time.sleep(int(lines[2].lstrip().rstrip()))
    continue
print(course_db)
